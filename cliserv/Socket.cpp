...
Socket::Socket(int type) : _sockfd(::socket(AF_INET, type, /*protocol*/0)) {
#if defined(IGNORE_SIGPIPE) && defined(SO_NOSIGPIPE)
  int set = 1;
...
Socket* ServerSocket::accept() {
  ...
  else return new Socket(SOCK_STREAM, sock_com);
}
...
void SocketBuffer::setSeparators(int inputSeparator, int outputSeparator) {
  _inSep = inputSeparator;
  _outSep = outputSeparator;
}

void SocketBuffer::getSeparators(int& inputSeparator, int& outputSeparator) const {
  inputSeparator = _inSep;
  outputSeparator = _outSep;
}
...
ssize_t SocketBuffer::readLine(string& str) {
  ...
  while (true) {
    ...
    // - received = 0: nothing received (shutdown or empty message)  !!! verif shutdown envoei 0 !!!
